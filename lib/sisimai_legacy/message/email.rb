module SisimaiLegacy
  class Message
    # SisimaiLegacy::Message::Email convert bounce email text to data structure. It
    ...
    class Email
      ...
      RFC822Head = SisimaiLegacy::RFC5322.HEADERFIELDS
      RFC3834Set = SisimaiLegacy::RFC3834.headerlist.map(&:downcase)
      ...
      DefaultSet = SisimaiLegacy::Order::Email.another
      SubjectTab = SisimaiLegacy::Order::Email.by('subject')
      ExtHeaders = SisimaiLegacy::Order::Email.headers
      ...
      def self.make(argvs)
        ...
        tobeloaded = SisimaiLegacy::Message::Email.load(methodargv)

        # 1. Split email data to headers and a body part.
        aftersplit = SisimaiLegacy::Message::Email.divideup(email)
        ...
        processing['header'] = SisimaiLegacy::Message::Email.headers(aftersplit['header'], headerargv)

        # 3. Check headers for detecting MTA modules
        if headerargv['tryonfirst'].empty?
          headerargv['tryonfirst'] += SisimaiLegacy::Message::Email.makeorder(processing['header'])
        end
        ...
        bouncedata = SisimaiLegacy::Message::Email.parse(methodargv)
        ...
        processing['rfc822'] = if rfc822part.is_a? ::String
                                 # The value returned from SisimaiLegacy::Bite::Email::* modules
                                 SisimaiLegacy::Message::Email.takeapart(rfc822part)
                               else
                                 # The value returned from SisimaiLegacy::Bite::JSON::* modules
                                 rfc822part
                               end
        return processing
      end
      ...
      def self.makeorder(heads)
        ...
        # SisimaiLegacy::Order->by('subject') method
      ...
      def self.takeapart(heads)
        ...
        while e = hasdivided.shift do
          ...
          if cv = e.match(/\A([-0-9A-Za-z]+?)[:][ ]*(.*)\z/)
            ...
            if SisimaiLegacy::MIME.is_mimeencoded(e)
        ...
        if takenapart['subject']
          ...
          if SisimaiLegacy::String.is_8bit(v)
            ...
            if mimeborder['subject']
              ...
              v.split(BorderLine).each do |m|
                ...
                r << m if SisimaiLegacy::MIME.is_mimeencoded(m)
              end
            else
              # Subject line is not MIME encoded
              r << v
            end
            v = SisimaiLegacy::MIME.mimedecode(r)
          end
      ...
      def self.parse(argvs)
        ...
        if mesgformat.start_with?('text/plain', 'text/html')
          ...
          if ctencoding == 'base64'
            ...
            bodystring = SisimaiLegacy::MIME.base64d(bodystring)

          elsif ctencoding == 'quoted-printable'
            # Content-Transfer-Encoding: quoted-printable
            bodystring = SisimaiLegacy::MIME.qprintd(bodystring)
          end

          if mesgformat.start_with?('text/html;')
            # Content-Type: text/html;...
            bodystring = SisimaiLegacy::String.to_plain(bodystring, true)
          end
        else
          # NOT text/plain
          if mesgformat.start_with?('multipart/')
            # In case of Content-Type: multipart/*
            p = SisimaiLegacy::MIME.makeflat(mailheader['content-type'], bodystring)
        ...
        scannedset = nil
          ...
          while true
            ...
            # 4. SisimaiLegacy::Bite::Email::*
            # 5. SisimaiLegacy::RFC3464
            # 6. SisimaiLegacy::RFC3834
            if SisimaiLegacy::ARF.is_arf(mailheader)
              # Feedback Loop message
              scannedset = SisimaiLegacy::ARF.scan(mailheader, bodystring)
